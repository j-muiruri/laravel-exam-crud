...
const app = new Vue({
    el: '#app',
    data: {
        newExam: {'question': '', 'option1': '', 'option2': '', 'option3': '', 'option4': '', 'category': ''},
        hasError: true,
        exams: [],
     },
    //  Create exams
     createExam: function createExam() {
        ...
        if(input['question'] == '' || input['option1'] == '' || input['option1'] == '' || input['option2'] == '' || input['option3'] == '' || input['option4'] == ''|| input['category'] == '') {
            ...
            axios.post('/create', input).then(function(response){
                _this.newExam = {'question': '', 'option1': '', 'option2': '', 'option3': '', 'option4': '', 'category': ''}
                _this.getExams();
            }).catch(error=>{
    ...
    //   Edit Exam Question
      editExam: function(){
        ...
        var model = document.getElementById('myModal').value;
         axios.post('/update/' + id_val_1.value, {val_1: question_val_1.value, val_2: option1_val_1.value, val_3: option2_val_1.value, val_4: option3_val_1.value, val_5: option4_val_1.value,  val_6: category_val_1.value})
           .then(response => {
             _this.getExams();
           });
 },
